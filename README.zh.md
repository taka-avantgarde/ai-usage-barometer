...
| 显示 Codex 第1个额度 | 显示/隐藏 Codex 返回的第一个额度窗口 |
| Codex 第1个额度百分比 | 显示或隐藏其百分比 |
| 显示 Codex 第2个额度 | 若存在则显示或隐藏第二个额度窗口 |
| Codex 第2个额度百分比 | 显示或隐藏其百分比 |
| 刷新间隔 | 1、3 或 5 分钟 |
...
**Codex** 读取安装程序放在 `~/SwiftBar/.ai-usage-barometer/` 的本地助手 `codex-usage.sh` 的输出。其额度窗口的数量和名称都是动态的——套餐变化可能让 `5h`/`7d` 变成类似 `30d` 的名称——因此上面两个开关分别跟随 Codex 实际返回的第一、第二个窗口，设置面板也会自动更新为对应的名称。

## 疑难排解

**出现 Claude 警告** — 打开下拉菜单查看具体原因。*Sign in to Claude Code again* 表示 OAuth 令牌已过期，请通过 Claude Code 的 CLI 或 IDE 扩展重新登录。**这不是 Claude 桌面应用**：登录 Claude.app 不会刷新此令牌。*Rate limited* 会自行解除；插件会等待 15 分钟并显示剩余时间。
