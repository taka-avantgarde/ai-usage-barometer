...
| Codex 첫 번째 한도 표시 | Codex가 처음 반환하는 한도 표시/숨김 |
| Codex 첫 번째 한도 퍼센트 | 해당 퍼센트 표시/숨김 |
| Codex 두 번째 한도 표시 | 두 번째 한도가 있으면 표시/숨김 |
| Codex 두 번째 한도 퍼센트 | 해당 퍼센트 표시/숨김 |
| 새로고침 간격 | 1분, 3분, 5분 |
...
**Codex**는 설치 프로그램이 `~/SwiftBar/.ai-usage-barometer/`에 배치한 로컬 헬퍼 `codex-usage.sh`의 출력에서 읽습니다. 한도 창은 개수와 이름 모두 동적입니다 — 요금제가 바뀌면 `5h`/`7d`가 `30d`처럼 바뀔 수 있습니다 — 그래서 위 두 토글은 Codex가 실제로 반환하는 첫 번째·두 번째 한도를 그대로 따라가며, 설정 화면의 표기도 실제 이름에 맞춰 자동으로 바뀝니다.

## 문제 해결

**Claude 경고가 표시됨** — 드롭다운을 열어 원인을 확인하세요. *Sign in to Claude Code again*은 OAuth 토큰이 만료되었다는 뜻으로, Claude Code CLI나 IDE 확장에서 다시 로그인해야 합니다. **Claude 데스크톱 앱이 아닙니다**: Claude.app에 로그인해도 이 토큰은 갱신되지 않습니다. *Rate limited*는 시간이 지나면 자동으로 해소됩니다. 플러그인이 15분 대기하며 남은 시간을 표시합니다.
